...
module ChannelNegotiation where 
    ...
    liftIO $ scotty negotiationport $ do
	    ...
	    post "/" $ do
	     ...
	     -- myprint' ("Data received on port: " ++ (show port)) 1
	      ...
	      let jj = AD.jsonEitherDecode (LazyEncoding.encodeUtf8 a) :: Either String AD.Shared
		   ...
		   (Right (WPort p)) -> do 
		                     ...
		                     case eitherCAResp of
		                        ...
		                        (Right caResp) 	-> do
		                        		     myprint' "Sending response" 1
		                                             json caResp 
	           (Right (WVChanChan i) ) -> do
	                             chanls <-liftIO $ takeTMVar chanlsTMVar
	                             chanls' <- ensureListeningOn i chanls me
	           ...
	           (Right _)       -> do 
                		     let str = ("Recieved something that wasn't a WPort. No! I refuse: " ++ err)
                                     --myprint' str 1    --report error: console
                                     text (LazyText.pack str) --report error: client
ensureListeningOn :: Int -> [(String, (Channel, MVar [Armored]) )] -> Entity  -> IO [(String, (Channel, MVar [Armored]) )] 
ensureListeningOn id [] me = do
			  mvarMssgs <- newEmptyMVar
			  return [(("VChanWith" ++ id),(Channel 
ensureListeningOn id chans me = do
			  mvarMssgs <- newEmptyMVar
			  return [(("VChanWith" ++ i),
