...
void VM_MonitorAction::doit() {
	tty->print("\nJG: Reached VM_MonitorAction:doit");
	tty->print("\nJG: VM_MonitorAction(), calling trace_stack()\n");

	ALL_JAVA_THREADS(p) {
    		//tc->do_thread(p);
		//p->trace_stack();
		//DM - Change Start
	        //Both of these should be specified, these are just filler values
	...
	tty->print("\nJG: VM_MonitorAction(), returned from trace_stack()");
}
//JG - Change Stop
