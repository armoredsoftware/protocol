...
void Thread::print_on_error(outputStream* st, char* buf, int buflen) const {
  ...
  //JG - Change Start
  else if (is_Talk_thread())                st->print("TalkThread");
  //JG - Change End
...
//JG - Change Start
std::string JavaThread::trace_stack_from2(vframe* start_vf) {
  std::string temp = "";
  ResourceMark rm;
  int vframe_no = 1;
  for (vframe* f = start_vf; f; f = f->sender() ) {
    if (f->is_java_frame()) {
      temp += javaVFrame::cast(f)->get_activation(vframe_no++) + "\n";
    } else {
      { char *cstring; asprintf(&cstring, "ELSE!"); temp += cstring; }
      //f->print();
    }
    if (vframe_no > StackPrintLimit) {
      { char *cstring; asprintf(&cstring, "...<more frames>..."); temp += cstring; }
      //tty->print_cr("...<more frames>...");
      return temp;
    }
  }
  return temp;  //added, recheable?
}
std::string JavaThread::trace_stack2() {
  if (!has_last_Java_frame()) return "";
  ResourceMark rm;
  HandleMark   hm;
  RegisterMap reg_map(this);
  std::string temp = trace_stack_from2(last_java_vframe(&reg_map));
  return temp;
}
std::string JavaThread::trace_stack_from3(vframe* start_vf) {
  std::string temp = "";
  ResourceMark rm;
  int vframe_no = 1;
  for (vframe* f = start_vf; f; f = f->sender() ) {
    if (f->is_java_frame()) {
      temp += javaVFrame::cast(f)->get_activation_PConly(vframe_no++) + "\n";
      break; //only do first PC
    }
    if (vframe_no > StackPrintLimit) {
      return temp;
    }
...
std::string JavaThread::trace_stack3() {
  if (!has_last_Java_frame()) return "";
  ResourceMark rm;
  HandleMark   hm;
  RegisterMap reg_map(this);
  std::string temp = trace_stack_from3(last_java_vframe(&reg_map));
  return temp;
}
//JG - Change End
//JG - Change Start
void JavaThread::var_trace_stack_from(vframe* start_vf, int index, char* desired_frame, char* result) {
  ResourceMark rm;
  int vframe_no = 1;
  //char result[200];
  for (vframe* f = start_vf; f; f = f->sender() ) {
    if (f->is_java_frame()) 
    {
      /*result = */javaVFrame::cast(f)->var_print(index, desired_frame, result);
      //return;
    } 
    else 
    {
...
void  JavaThread::var_trace_stack(int index, char* desired_frame, char* result) {
  //char result[200];
  if (!has_last_Java_frame()) return;
  ResourceMark rm;
  HandleMark   hm;
  RegisterMap reg_map(this);
  /*result = */var_trace_stack_from(last_java_vframe(&reg_map), index, desired_frame, result);
  return;
}
//JG - End Change
