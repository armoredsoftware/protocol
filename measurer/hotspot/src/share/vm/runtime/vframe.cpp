...
//JG - Change Start
std::string javaVFrame::get_value() const {
  std::string temp = "";
  methodOop  m = method();
  klassOop   k = m->method_holder();
  { char *cstring; asprintf(&cstring, "frame( sp=" INTPTR_FORMAT ", unextended_sp=" INTPTR_FORMAT ", fp=" INTPTR_FORMAT ", pc=" INTPTR_FORMAT ")", _fr.sp(),  _fr.unextended_sp(), _fr.fp(), _fr.pc()); temp += cstring; }
  //tty->print_cr("frame( sp=" INTPTR_FORMAT ", unextended_sp=" INTPTR_FORMAT ", fp=" INTPTR_FORMAT ", pc=" INTPTR_FORMAT ")",
  //              _fr.sp(),  _fr.unextended_sp(), _fr.fp(), _fr.pc());
  { char *cstring; asprintf(&cstring, "%s.%s", Klass::cast(k)->internal_name(), m->name()->as_C_string()); temp += cstring; }
 // tty->print("%s.%s", Klass::cast(k)->internal_name(), m->name()->as_C_string());
  ...
  if (!m->is_native()) {
    ...
    int        line_number = m->line_number_from_bci(bci());
    if (source_name != NULL && (line_number != -1)) {
      { char *cstring; asprintf(&cstring, "(%s:%d)", source_name->as_C_string(), line_number); temp += cstring; }
      //tty->print("(%s:%d)", source_name->as_C_string(), line_number);
    }
  } else {
    { char *cstring; asprintf(&cstring, "(Native Method)"); temp += cstring; }
    //tty->print("(Native Method)");
  }
  // Check frame size and print warning if it looks suspiciously large
  if (fr().sp() != NULL) {
...
std::string javaVFrame::get_activation(int index) const {
  // frame number and method
  std::string temp = "";
  { char *cstring; asprintf(&cstring, " %2d - ", index); temp += cstring; }
  //tty->print("%2d - ", index);
  temp += get_value();//((vframe*)this)->get_value();
  //{ char *cstring; asprintf(&cstring, "\n"); temp += cstring; }
  //tty->cr();

  return temp;
}
std::string javaVFrame::get_value_PConly() const {
  std::string temp = "";
  methodOop  m = method();
  klassOop   k = m->method_holder();
  { char *cstring; asprintf(&cstring, "pc=" INTPTR_FORMAT "", _fr.pc()); temp += cstring; }
  return temp;
}
std::string javaVFrame::get_activation_PConly(int index) const {
  // frame number and method
  std::string temp = "";
  { char *cstring; asprintf(&cstring, " %2d - ", index); temp += cstring; }
  temp += get_value_PConly();

  return temp;
}
//JG - Change End
//DM - Change Start
void javaVFrame::var_print(int index, char* desired_frame, char* result) const {
  // frame number and method

  methodOop m = method();
  char* method_name = m->name()->as_C_string();

  if(!strcmp(method_name, desired_frame))
  {
    //printf("Method name is: %s \n", method_name);
    //print_stack_values("All Locals", locals());

    //Pull all locals into values
    StackValueCollection* values = locals();
    //Get specified local variable
    if (index < values->size() && index >= 0)
    {
      //get the particular value we're interested in 
      StackValue* value = values->at(index);

      //now get a string containing the same value
      char tmp[200];
      char formatting[200];
      if(value->type() == T_INT)
      {
        sprintf(result, "Local %2d =", index);
	intptr_t i;
	i = value->get_int();
        sprintf(tmp, "%d(int), %f(float), %x(hex)",  *(int *)&i, *(float *)&i,  *(int *)&i);
	strcat(result, tmp);
	if(index+1 < values->size() && values->at(index+1)->type() == T_INT)
	{
	    sprintf(formatting, "\n\t%s(long), %s(double), %s(longhex)\n", INT64_FORMAT, "%.15e", PTR64_FORMAT);
	    sprintf(tmp, formatting, values->long_at(index), values->double_at(index), values->long_at(index)); 
	    strcat(result, tmp);
	}
	//printf("Local within vframe.cpp: %s\n", result);
	//printf("size of string: %i\n", strlen(result));
      }
      else if (value->type() == T_OBJECT)
      {
	sprintf(formatting, " <%s>", INTPTR_FORMAT);
	Handle _o;
	_o = value->get_obj();
	sprintf(tmp, formatting, (address)_o());
	strcat(result, tmp);
        //sprintf(result, "UNHANDLED CASE!\n");
      }
    }
    else
    {
      //index provided is not valid
      sprintf(result, "The local variable index provided is not valid\n");
    }
  }
  //  if(result[0] == 0)
    //sprintf(result, "The specified variable was not found.\n");
}
//DM - Change Stop
