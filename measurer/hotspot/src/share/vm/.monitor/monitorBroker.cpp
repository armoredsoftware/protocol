#include "compiler/monitorBroker.hpp"
...
void CompileBroker::montor_task_request_1(/*???...*/) {

  // do nothing if monitor thread(s) is not available
  if (!_initialized ) {
    return;
  }

  // If this method is already in the compile queue, then we do not block the current thread.
  ...
  MonitorTask* queue  = monitor_queue(comp_level);
  ...
  // Acquire our lock.
  {
    ...
    uint monitor_id = assign_monitor_id(method, osr_bci);
    if (monitor_id == 0) {
      // The compilation falls outside the allowed range.
      return;
    }

    task = create_monitor_task(queue,
                               monitor_id,
				/*???...*/);
  }
...
// ------------------------------------------------------------------
// MonitorBroker::create_monitor_task
//
// Create a MonitorTask object representing the current request for
// monitoring.  Add this task to the queue.
MonitorTask* MonitorBroker::create_monitor_task(MonitorQueue* queue,
                                              int           monitor_id,
                                              /*???...*/) {
  MonitorTask* new_task = allocate_task();
  new_task->initialize(monitor_id, /*???...*/);
  queue->add(new_task);
  return new_task;
}
