...
//JG - Change Start#include <sys/socket.h>
#include <sys/types.h>
...
static void signal_thread_entry(JavaThread* thread, TRAPS) {
  ...
  tty->print("\nJG: Entered signal_thread_entry()\n");
  while (true) {
    int sig;
    {
    ...
    switch (sig) {
      ...
      //JG - CHANGE START
      case 10: case 16: case 17: case 30: {
	for (int r=0;r<1;r++) {
	//tty->print("\nJG: signal_thread_entry(), case 10, custom signal");

	//Prepare VM Operation
	VM_MonitorAction op;
	op.signal = sig;

	//get arguments (for 17)
	if (sig == 17) {
		int sockfd = 0, n = 0;
    		char recvBuff[1024];
		...
		int request_size = request.length();
	   	char sendBuff[request_size];//[1025]

		//tty->print("\nWriting request..");
		snprintf(sendBuff, sizeof(sendBuff), "%s", request.c_str());
		...
		for (int i=0; i<sizeof(recvBuff); i++) {
			if (recvBuff[i]==',')
				...
				if (s==0) method += recvBuff[i];//method.append(1, 'a');
				else local += recvBuff[i];//local.append(1, 'a');
		}
		//tty->print("\nn=%d, method=%s, local=%s\n", n, method.c_str(), local.c_str());

		op.method = new char[method.length()];
		strcpy (op.method,method.c_str());
		op.local = atoi(local.c_str());
	...
	//Print Times
    	...
    	//tty->print("JG: monitor task total:\t%fms\n", time_taken);
	double time_taken_SP = 1000 * ((double)op.elapsed)/CLOCKS_PER_SEC;
    	//tty->print("JG: monitor task in SP:\t%fms\n", time_taken_SP);
	tty->print("%f\t%f\n", time_taken, time_taken_SP);
	...
	int result_size = result.length();
   	char sendBuff[result_size];//[1025]
        //snprintf(sendBuff, sizeof(sendBuff), "%.24s\r\n", ctime(&ticks));
	//tty->print("\\START OF SEND \"\n%s\n\" \\END OF SEND", result.c_str());
	snprintf(sendBuff, sizeof(sendBuff), "%s", result.c_str());
        write(sockfd, sendBuff, strlen(sendBuff));
	
        close(sockfd);

	//tty->print("\nJG: signal_thread_entry(),finished servicing signal 10\n");	
...
void os::signal_init() {
  if (!ReduceSignalUsage) {
    ...
    //JG - CHANGE START
    os::signal(10, os::user_handler());
    os::signal(16, os::user_handler());
    os::signal(17, os::user_handler());
    os::signal(30, os::user_handler());
    //JG - CHANGE END
  }
}
