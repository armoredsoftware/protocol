...
//DM - Edit Start
/*StackValueCollection* interpretedVFrame::single_local(int index) const {
int length = method()->max_locals();
    ...
    StackValueCollection* local_var = new StackValueCollection(length);
    ...
    {
      intptr_t *addr = locals_addr_at(index);
	...
	sv = new StackValue(*addr);
      ...
      assert(sv != NULL, "make sure we have a valid value");  //this might take care of the check actually
      local_var->add(sv);
    }
    else
    {
      printf("The specified index is not valid");
    }
    return local_var;
    }*/
//DM - Edit Stop
