...
struct ME_CG * 
ME_CG_from_frame(struct frame_info *frame, struct ME_FT * ft)
{
  if (get_frame_type (frame) == DUMMY_FRAME
      || get_frame_type (frame) == SIGTRAMP_FRAME
      || get_frame_type (frame) == ARCH_FRAME)
...
BE_get_call_stack_as_CG (char *count_exp, int show_locals, int no_filters,
		      int from_tty, struct ME_CG ** cg, struct ME_FT * ft)
{
  struct frame_info *fi;
  int count;
  int i;
  ...
  if (no_filters ||  result == EXT_LANG_BT_NO_FILTERS)
      ...
      for (i = 0, fi = trailing; fi && count--; i++, fi = get_prev_frame (fi))
	{
	  ...
	  struct ME_CG * cg_frame = ME_CG_from_frame (fi, ft);
	  ME_CG_add_child(cg_frame, last_cg);
